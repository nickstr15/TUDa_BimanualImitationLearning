...
$> python -m robomimic_ext.scripts.train -c config.json
...
def train(config: BaseConfig, device: torch.device):
    ...
    print(config)
    print("")
    log_dir, ckpt_dir, video_dir = train_utils.get_exp_dir(config)
    ...
    ou.initialize_obs_utils_with_config(config)
    ...
    env_meta = fu.get_env_metadata_from_dataset(dataset_path=config.train.data)
    shape_meta = fu.get_shape_metadata_from_dataset(
    ...
    if config.experiment.rollout.enabled:
        ...
        for env_name in env_names:
            ...
            env = eu.wrap_env_from_config(env, config=config)  # apply environment wrapper, if applicable
            envs[env.name] = env
            print(envs[env.name])
    ...
    if config.experiment.logging.log_wandb:
        ...
        if WANDB_API_KEY is None:
            print("WANDB_API_KEY is not set. Please set it in utils/wandb.py. Ignoring wandb logging.")
    ...
    # load training data
    train_set, valid_set = train_utils.load_data_for_training(
        config, obs_keys=shape_meta["all_obs_keys"])
    train_sampler = train_set.get_dataset_sampler()
    print("\n============= Training Dataset =============")
    print(train_set)
    print("")
    ...
    if config.train.hdf5_normalize_obs:
        obs_normalization_stats = train_set.get_obs_normalization_stats()
    ...
    if config.experiment.validate:
        ...
        valid_sampler = valid_set.get_dataset_sampler()
    ...
    for epoch in range(1, config.train.num_epochs + 1):  # epoch numbers start at 1
        step_log = train_utils.run_epoch(
            model=model,
        ...
        if config.experiment.validate:
            with torch.no_grad():
                step_log = train_utils.run_epoch(model=model, data_loader=valid_loader, epoch=epoch, validate=True,
        ...
        if config.experiment.rollout.enabled and (epoch > config.experiment.rollout.warmstart) and rollout_check:
            ...
            all_rollout_logs, video_paths = train_utils.rollout_with_stats(
            ...
            updated_stats = train_utils.should_save_from_rollout_logs(
        ...
        if should_save_ckpt:
            train_utils.save_model(
                ...
                ckpt_path=str(os.path.join(ckpt_dir, epoch_ckpt_name + ".pth")),
                obs_normalization_stats=obs_normalization_stats,
            )
...
def prep_training_run(args: Namespace):
    if args.config is not None:
        json_config_path = path_completion(args.config, RM_EXP_CONFIG_DIR)
        ext_cfg = json.load(open(json_config_path, 'r'))
...
def main(args: Namespace):
    config, device = prep_training_run(args)
