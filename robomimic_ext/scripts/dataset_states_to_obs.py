...
This script is an extension of https://github.com/ARISE-Initiative/robomimic/blob/9273f9cce85809b4f49cb02c6b4d4eeb2fe95abb/robomimic/scripts/dataset_states_to_obs.py
...
def extract_trajectory(
    ...
    Args:
        ...
        done_mode (int): how to write done signal. If 0, done is 1 whenever s' is a
            ...
            If 2, do both.
        camera_names (list of str): camera names to use for image observations
        camera_height (int): height of image observations
        camera_width (int): width of image observations
    """
    ...
    # maybe add in intrinsic and extrinsic for all cameras
    camera_info = None
    is_robosuite_env = eu.is_robosuite_env(env=env)
    if is_robosuite_env:
    ...
    traj["obs"] = tu.list_of_flat_dict_to_dict_of_list(traj["obs"])
    traj["next_obs"] = tu.list_of_flat_dict_to_dict_of_list(traj["next_obs"])
...
def get_camera_info(
    ...
    """
    Helper function to get camera intrinsic and extrinsic for cameras being used for observations.
    """
    assert eu.is_robosuite_env(env=env)
...
def dataset_states_to_obs(args):
    ...
    is_robosuite_env = eu.is_robosuite_env(env_meta)

    # list of all demonstration episodes (sorted in increasing number order)
    f = h5py.File(input_path, "r")
    demos = list(f["data"].keys())
    idxs = np.argsort([int(elem[5:]) for elem in demos])
    demos = [demos[i] for i in idxs]
    ...
    output_path = path_completion(args.output_path, DATASET_DIR)
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="path to input hdf5 dataset. if relative path, it is relative to the directory `data\demonstrations`",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="name of output hdf5 dataset. If relative path, it is relative to the directory `data\datasets`",
    )
