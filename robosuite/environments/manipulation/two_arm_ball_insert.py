...
from robosuite.models.objects import HammerObject, Bin, BallObject
...
from robosuite.utils.paths import path_completion


class TwoArmBallInsert(TwoArmEnv):
    """
    This class corresponds to a bimanual task, requiring the robot to pick up a
    ball, and place it in a bin. The task is considered successful if the ball is
    placed in the bin.
    ...
    def __init__(
        ...
        # settings for bin
        self._ball_min_size = np.array([ball_min_size]) if type(ball_min_size) in [int, float] else np.array(ball_min_size)
        self._ball_max_size = np.array([ball_max_size]) if type(ball_max_size) in [int, float] else np.array(ball_max_size)
    ...
    def reward(self, action=None):
        ...
        Sparse un-normalized reward:
            0 if ball is not in the bin
            1 if ball is in the bin

        Un-normalized summed components if using reward shaping:
            Distance: in [-1, 0], the distance between the ball and the bin, normalized
                by initial distance (-1 at start), 0 when ball is in the bin
            Success: 0 if ball is not in the bin, 1 if ball is in the bin
        ...
        if self.reward_shaping:
            ...
            dist = np.linalg.norm(self._ball_pos - self._bin_pos)
            normed_dist = dist / self._initial_ball_bin_dist
    ...
    def _load_model(self):
        ...
        self.model = ManipulationTask(
            ...
            mujoco_objects = [self.ball, self.bin],
        )
    ...
    def _get_placement_initializer(self):
        ...
        objects = [self.ball, self.bin]
        x_centers = [
            -0.15,
            -0.15+1.3*(self.ball.horizontal_radius+0.5*self.bin.bin_size[0])
        ]
        x_tol = self.position_tol[0]
        y_tol = self.position_tol[1]
        rot_tol = self.orientation_tol
        rot_axes = ["x", "z"]
        for obj, x, r_axis in zip(
            objects, x_centers, rot_axes
        ):
    ...
    def _setup_observables(self):
        ...
        if self.use_object_obs:
            ...
            def ball_pos(_):
                return np.array(self._ball_pos)
            ...
            sensors = [ball_pos, bin_pos, bin_quat]
    ...
    def _reset_internal(self):
        ...
        self._initial_ball_bin_dist = np.linalg.norm(self._ball_pos - self._bin_pos)
    ...
    def _ball_pos(self):
        ...
        """
        return self.sim.data.body_xpos[self.ball_body_id]
    ...
    @property
    def _ball_in_bin(self):
        """
        Returns True if the ball is in the bin, False otherwise.
        """
...
if __name__ == "__main__":
    from robosuite.environments.utils.visualize import visualize_static

    visualize_static("TwoArmBallInsert", robots="Baxter")
